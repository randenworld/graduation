...
class Conv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bottleneck = False,group=1) -> None:
        super(Conv3DBlock, self).__init__()
        self.conv1 = nn.Conv3d(in_channels= in_channels, out_channels=out_channels//2, kernel_size=(3,3,3), padding=1,groups=group)
        self.bn1 = nn.BatchNorm3d(num_features=out_channels//2)
        self.conv2 = nn.Conv3d(in_channels= out_channels//2, out_channels=out_channels, kernel_size=(3,3,3), padding=1,groups=group)
...
class UpConv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, res_channels=0, last_layer=False, num_classes=None,group=1) -> None:
        super(UpConv3DBlock, self).__init__()
        assert (last_layer==False and num_classes==None) or (last_layer==True and num_classes!=None), 'Invalid arguments'
        self.upconv1 = nn.ConvTranspose3d(in_channels=in_channels, out_channels=in_channels, kernel_size=(2, 2, 2), stride=2,groups=group)
        self.relu = nn.ReLU()
        self.bn = nn.BatchNorm3d(num_features=in_channels//2)
        self.conv1 = nn.Conv3d(in_channels=in_channels+res_channels, out_channels=in_channels//2, kernel_size=(3,3,3), padding=(1,1,1),groups=group)
        self.conv2 = nn.Conv3d(in_channels=in_channels//2, out_channels=in_channels//2, kernel_size=(3,3,3), padding=(1,1,1),groups=group)
...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[FIRST_CHANNEL, FIRST_CHANNEL*2, FIRST_CHANNEL*4], bottleneck_channel=FIRST_CHANNEL*8,) -> None:
