...
def load_dicom_series(dicom_files):
    ...
    if intercept < -1000:
        image_data = image_data + intercept + 1000  # 調整 HU 值 
        intercept = -1000
    image_data2 = image_data * slope + intercept
# 取得 WL / WW 參數
    print(f"Window Center: {slices[0].WindowCenter}, Window Width: {slices[0].WindowWidth} , tpye: {type(slices[0].WindowCenter)}")
    if type(slices[0].WindowCenter) is pydicom.multival.MultiValue:  # 檢查 WindowCenter 是否為列表
        # 如果 WindowCenter 有多個值，取第一個值
