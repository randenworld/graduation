...
from unet_3D import build_unet3d
import datetime
import random
...
def predict_2_5d_single_patient(model, image_paths):
...
def train():
    ...
    dir_list = [os.path.join(DATASET,x) for x in os.listdir(DATASET)]
    ...
    plt.tight_layout()
    plt.savefig(f'log_2d/{DATASET.split("/")[-1]}_{now_time}_train_data.png')
    plt.close()
